...
const AdvertisingModel = {
  ...
  updateAdvertising: async (advertisingId, updateData) => {
    try {
      ...
      return await Advertising.findByIdAndUpdate(advertisingId, updateData, {
        new: true,
        runValidators: true,
      });
    } catch (error) {
  ...
  getFeaturedProperties: async (limit = 4) => {
    try {
      ...
      const activeAds = await Advertising.find({
        ...
        .populate("propertyId")
        .sort({ priority: -1, lastFeatured: 1 });

      // Filter out any ads where propertyId is null or not active
      const validAds = activeAds.filter(
        (ad) => ad.propertyId && ad.propertyId.status === "active"
      );
