...
router.get("/my-properties", requireAuth, requireSeller, async (req, res) => {
  try {
    ...
    const propertiesWithAgents = Array.isArray(sellerProperties)
      ? await Promise.all(
          sellerProperties.map(async (property) => {
            let agent = await AgentModel.getAgentById(property.agent);
...
router.get("/my-transactions", requireAuth, requireSeller, async (req, res) => {
  try {
    const transactions = await TransactionModel.getTransactionsBySeller(
      req.user._id
    );
...
router.get("/my-purchases", requireAuth, requireBuyer, async (req, res) => {
  try {
    ...
    const propertiesWithAgents = Array.isArray(buyerProperties)
      ? await Promise.all(
          buyerProperties.map(async (property) => {
            let agent = await AgentModel.getAgentById(property.agent);
...
router.get("/tag/:tag", async (req, res) => {
  try {
    ...
    if (!["sale", "rent"].includes(tag)) {
      ...
      return res
        .status(400)
        .render("error", { message: "Invalid tag parameter" });
    }
  ...
  async (req, res) => {
    try {
      ...
      console.log("Form data received:", {
        ...
        amenitiesCount: Array.isArray(amenities)
          ? amenities.length
          : amenities
          ? 1
          : 0,
      ...
      try {
        ...
        if (addressToGeocode && addressToGeocode.trim() !== "") {
          ...
          const geocodeResult =
            await geocodingService.getCoordinatesFromAddress(addressToGeocode);
      ...
      const newProperty = await PropertyModel.addProperty(
        propertyData,
        req.user._id
      );
      ...
      if (
        ...
        res.redirect(
          `/dashboard?section=my-properties&success=${encodeURIComponent(
            successMessage
          )}`
        );
      }
...
router.post("/:id/purchase", requireAuth, requireBuyer, async (req, res) => {
  try {
    ...
    if (buyer.accountBalance < priceValue) {
      ...
      return res.redirect(
        `/property/${req.params.id}?error=insufficientFunds&amountNeeded=${amountNeeded}`
      );
    }
    ...
    res.redirect(
      `/property/${
        req.params.id
      }?error=purchaseError&message=${encodeURIComponent(error.message)}`
    );
  }
});
  ...
  async (req, res) => {
    try {
      ...
      if (!property) {
        ...
        return res
          .status(404)
          .render("error", { message: "Property not found" });
      }
      ...
      try {
        ...
        const cancellationPromise = PropertyModel.cancelRental(
          propertyId,
          req.user._id
        );
        ...
        console.error(
          "Error during rental cancellation operation:",
          operationError
        );
        ...
        if (operationError.message === "Operation timed out") {
          if (req.xhr || req.headers.accept.indexOf("json") > -1) {
            return res.status(408).json({
              success: false,
              message:
                "The cancellation request is taking longer than expected. Please refresh your dashboard in a moment to check the status.",
            });
          }
          return res.redirect(
            "/dashboard?section=my-purchases&warning=The cancellation request is taking longer than expected. Please refresh after a few moments to check the status."
          );
        }
        ...
        if (
          ...
          console.log(
            "Non-critical error during cancellation, attempting to verify if cancellation succeeded"
          );
          ...
          try {
            ...
            if (property && property.status === "active" && !property.buyerId) {
              console.log(
                "Cancellation succeeded despite error, returning success"
              );
        ...
        if (req.xhr || req.headers.accept.indexOf("json") > -1) {
          return res.status(500).json({
            success: false,
            message:
              "Failed to cancel rental agreement: " + operationError.message,
          });
        }
  ...
  async (req, res) => {
    try {
      ...
      if (!verifiedAgents || verifiedAgents.length === 0) {
        ...
        return res.redirect(
          "/dashboard?error=No verified agents available&section=property-approval"
        );
      }
      ...
      const approvedProperty = await PropertyModel.approveProperty(
        id,
        req.user._id,
        notes,
        selectedAgentId
      ...
      return res.redirect(
        "/dashboard?success=Property has been approved&section=property-approval"
      );
    } catch (error) {
  ...
  async (req, res) => {
    try {
      ...
      if (!notes || notes.trim() === "") {
        ...
        return res.redirect(
          "/dashboard?error=Rejection notes are required&section=property-approval"
        );
      }

      // Reject the property
      const rejectedProperty = await PropertyModel.rejectProperty(
        id,
        req.user._id,
        notes
      );
      ...
      // Redirect back to the approval page with a success message
      return res.redirect(
        "/dashboard?success=Property has been rejected&section=property-approval"
      );
    } catch (error) {
  ...
  try {
    ...
    const matchedProperties = allProperties.filter(
      (property) =>
        ...
        (property.amenities &&
          property.amenities.some((amenity) => searchRegex.test(amenity)))
    );

    // Only return active and approved properties for comparison
    const availableProperties = matchedProperties.filter(
      (property) =>
  ...
  try {
    ...
    const neighborhoodData = {
      ...
      education: {
        ...
        colleges: [{ name: "City College", distance: "2.5 km", rating: 4.0 }],
        libraries: [{ name: "Community Library", distance: "1.0 km" }],
      },
      healthcare: {
        hospitals: [
        ...
        clinics: [{ name: "Family Clinic", distance: "0.7 km" }],
        pharmacies: [
      ...
      transportation: {
        ...
        metroStations: [{ name: "City Center Metro", distance: "1.5 km" }],
        railwayStations: [
      ...
      recreation: {
        ...
        gyms: [{ name: "Fitness Center", distance: "0.7 km" }],
        restaurants: [
    ...
    const nearbyProperties = allProperties
      .filter(
        (prop) =>
          ...
          prop.location &&
          prop.location.includes(city) // Filter by city
      )
...
router.get("/seller/approved", requireAuth, requireSeller, async (req, res) => {
  try {
    ...
    const approvedProperties =
      await PropertyModel.getApprovedPropertiesBySeller(req.user._id);
...
// Get properties for a specific agent together with distance from that agent
router.get(
  "/admin/agents/:id/properties-with-distance",
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
      ...
      const properties = await PropertyModel.getAgentProperties(agentId);
      ...
      const propertiesWithDistance = Array.isArray(properties)
        ? properties.map((property) => {
            ...
            if (
              agent.geolocation &&
              typeof agent.geolocation.latitude === "number" &&
              typeof agent.geolocation.longitude === "number" &&
              propertyObj.geolocation &&
              typeof propertyObj.geolocation.latitude === "number" &&
              typeof propertyObj.geolocation.longitude === "number"
            ) {
              distanceKm = AgentModel.calculateDistance(
                agent.geolocation.latitude,
                agent.geolocation.longitude,
                propertyObj.geolocation.latitude,
                propertyObj.geolocation.longitude
              );

              const serviceRadius =
                (agent.geolocation.serviceRadius &&
                  Number(agent.geolocation.serviceRadius)) ||
                50;

              if (typeof distanceKm === "number") {
                withinServiceRadius = distanceKm <= serviceRadius;
              }
            }
            ...
            return {
              ...
              geolocation: propertyObj.geolocation || null,
              distanceKm,
              withinServiceRadius,
            };
          })
      ...
      return res.json({
        ...
        agent: {
          ...
          name: agent.name,
          location: agent.location,
          geolocation: agent.geolocation || null,
          serviceRadius:
            (agent.geolocation && agent.geolocation.serviceRadius) || 50,
        },
        properties: propertiesWithDistance,
      });
    } catch (error) {
      console.error(
        "Error fetching properties for agent with distance info:",
        error
      );
      return res.status(500).json({
        success: false,
    ...
    try {
      ...
      const agent = await AgentModel.Agent.findByIdAndUpdate(
        id,
        {
          ...
          "geolocation.serviceRadius": serviceRadius
...
// Manually assign or change the agent for a specific property
router.post(
  "/admin/properties/:id/assign-agent",
  requireAuth,
  requireAdmin,
  async (req, res) => {
    try {
      ...
      if (!agentId) {
        return res.status(400).json({
          success: false,
          message: "Target agentId is required",
        });
      }
      ...
      const targetAgent = await AgentModel.getAgentById(agentId);
      if (!targetAgent) {
        return res.status(404).json({
          success: false,
      ...
      const previousAgentId =
        property.agent && property.agent.toString
          ? property.agent.toString()
          : property.agent || null;

      // Update the property's agent reference in the database
      const updatedProperty = await PropertyModel.updatePropertyAgent(
        propertyId,
        agentId
      );

      // Update listing counts for affected agents (best-effort)
      if (previousAgentId && previousAgentId !== agentId) {
        try {
          await AgentModel.updateAgentListingCount(previousAgentId);
        } catch (countError) {
          console.error(
            "Error updating previous agent listing count:",
            countError
          );
        }
      }
      ...
      try {
        ...
        console.error("Error updating new agent listing count:", countError);
      }

      return res.json({
        success: true,
        message: "Property agent assignment updated successfully",
        property: updatedProperty,
        agentId,
      });
    } catch (error) {
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      try {
        console.log(
          `Admin geocoding property ${id} with address: ${addressToGeocode}`
        );
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      console.log(
        `Starting batch auto-geocoding for ${propertyIds.length} properties`
      );
      ...
      for (const propertyId of propertyIds) {
        try {
          ...
          console.error(
            `❌ Failed to geocode property ${propertyId}:`,
            error.message
          );
          results.failed.push({
            propertyId,
      ...
      console.log(
        `Batch geocoding completed: ${results.successful.length} successful, ${results.failed.length} failed`
      );
