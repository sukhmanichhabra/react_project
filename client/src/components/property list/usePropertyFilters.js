  ...
  const applyFilters = useCallback(() => {
    ...
    let filtered = properties.filter((property) => {
      ...
      if (property.approvalStatus && property.approvalStatus !== "approved") {
        console.log(
          `❌ Property ${property.title} not approved: ${property.approvalStatus}`
        );
        return false;
      }
      ...
      // Status/Tag filter
      const statusMatch =
        filters.status === "all" || property.tag === filters.status;
      console.log(
        `🏷️ Status filter for ${property.title}: filters.status=${filters.status}, property.tag=${property.tag}, statusMatch=${statusMatch}`
      );
      if (!statusMatch) return false;

      // Location filter - handle null/undefined locations
      const propertyLocation =
        property.location || property.geolocation?.address || "";
      const locationMatch =
        !filters.location ||
      ...
      const priceMatch =
        propertyPrice >= filters.minPrice && propertyPrice <= filters.maxPrice;
      console.log(
        `💰 Price filter for ${property.title}: propertyPrice=${propertyPrice}, min=${filters.minPrice}, max=${filters.maxPrice}, priceMatch=${priceMatch}`
      );
      ...
      const propertyType =
        property.features?.type || property.type || property.propertyType || "";
      const typeMatch =
