...
const Verification = ({ agentProfile, onProfileUpdate }) => {
  ...
  const canUpload = !['verified', 'pending'].includes(verificationStatus);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!idProof || !license) {
      setUploadStatus({
        type: 'error',
        message: 'Please select both ID Proof and Real Estate License documents.'
      });
      return;
    }
    ...
    try {
      ...
      if (response.status === 200 || response.status === 302) {
        setUploadStatus({
          type: 'success',
          message: 'Documents uploaded successfully! Your verification is now pending review.'
        });
      ...
      if (error.response?.data?.error) {
        ...
        errorMessage = 'Please complete your profile before submitting documents.';
  ...
  return (
    <section id="verification">
      ...
      {/* Status Display */}
      <div
        className={`dash-verification-status-box ${getStatusColor(verificationStatus)}`}
      >
        <i
          className={`fas ${getStatusIcon(verificationStatus)}`}
        ></i>
        <div className="dash-status-text">
          <strong>
            Status: {getStatusText(verificationStatus)}
          </strong>
          <p>{verificationMessage || 'Complete your profile and submit documents for verification.'}</p>
        </div>
      </div>
        ...
        <div className="dash-rejection-notice">
          ...
          <div className="dash-rejection-body">
            ...
            <p className="rejection-help">
              Please review the feedback above and upload new documents that address the concerns mentioned.
            </p>
          </div>
        </div>
      )}
      ...
      {isPending && (
        <div className="dash-pending-notice">
          ...
          <div className="dash-pending-body">
            <p>Your documents have been submitted and are currently being reviewed by our verification team.</p>
            <ul>
              <li>Review typically takes 1-2 business days</li>
              <li>You will receive an email notification once the review is complete</li>
              <li>Please do not upload new documents while review is in progress</li>
            </ul>
          </div>
        </div>
      )}
      ...
      {canUpload && (
        ...
        >
          <h3>{hasBeenRejected ? 'Upload Updated Documents' : 'Submit Documents'}</h3>
          <p>
            {hasBeenRejected 
          ...
          <div className="dash-form-grid">
            <div className="dash-form-group">
              ...
              />
              <small>Required: Upload a clear image or PDF of your government-issued ID</small>
            </div>
          ...
          <div className="dash-form-grid">
            <div className="dash-form-group">
              ...
              <input
                ...
                disabled={isUploading}
              />
              <small>Optional: Business registration, company certificate, etc.</small>
            </div>
          ...
          <div className="dash-form-group full-width">
            ...
            <input
              ...
              disabled={isUploading}
            />
            <small>Optional: Upload up to 5 additional supporting documents</small>
            {additionalDocs.length > 0 && (
              <div className="selected-files">
                <p>Selected files: {additionalDocs.map(file => file.name).join(', ')}</p>
              </div>
            )}
          </div>
            ...
            >
              {isUploading ? (
                ...
                <>
                  ...
                  {hasBeenRejected ? 'Resubmit for Verification' : 'Submit for Verification'}
                </>
              )}
            </button>
          </div>
      ...
      {isVerified && (
        <div className="dash-verified-info">
          <div className="dash-verified-badge">
            ...
            <p>Your agent account has been verified. You now have access to all premium features and your profile displays the verified badge.</p>
            <div className="verified-benefits">
              ...
              <ul>
                ...
                <li><i className="fas fa-check"></i> Access to premium agent features</li>
                <li><i className="fas fa-check"></i> Higher visibility in search results</li>
                <li><i className="fas fa-check"></i> Enhanced credibility with clients</li>
              </ul>
            </div>
          </div>
        </div>
      ...
      {isPending && (
        <div className="dash-upload-disabled">
          ...
          <p>You cannot upload new documents while your current submission is being reviewed. Please wait for the review to complete.</p>
        </div>
      )}
    </section>
  );
